...
const ordenamientoQuicksort = (data) => {
  ...
  return [
    ...ordenamientoQuicksort(menores),
    pivot,
    ...ordenamientoQuicksort(mayores),
  ];
};
...
const ordenamientoRapido = (data) => {
  ...
  const menores = rest.filter(
    (item) => !desestructurar_fecha(item.createdAt, pivot.createdAt)
  );
  const mayores = rest.filter((item) =>
    desestructurar_fecha(item.createdAt, pivot.createdAt)
  );
...
export { ordernar_datos_fecha, ordenamientoRapido, ordenamiento };
