...
const autenticar = async (req, res) => {
    ...
    const verifyHasPassword = await funcionesBcrypt.verificarPassword(password, datos.docs[0].data().password)
    if (!verifyHasPassword) return res.status(403).json({ msg: "Password incorrecto" })
    ...
    try {
        ...
        await db_firebase.collection("Users").doc(id).set({ token: "", activo: false }, { merge: true });
...
const addUser = async (req, res) => {
    ...
    try {
        ...
        const usuario = await db_firebase.collection("Users").add({
            email, password: salt, token, activo, tipo, foto_perfil, name_user, acceso
        })
...
const restablecerPassword = async (req, res) => {
    ...
    try {
        ...
        const id = datos.docs[0].id;
        await db_firebase.collection("Users").doc(id).set({ token: "", acceso: true, activo: activo }, { merge: true });
...
const actulizarPassword = async (req, res) => {
    ...
    const id = datos.docs[0].id;
    let datos_actuales = {};
    if (password.trim() === "") return res.status(404).json({ msg: "Password vacio" })
    const salt = await funcionesBcrypt.encryptar(password)
